...
void Fuse::Renderer2D::DrawSprite(Fuse::Transform& transform, uint32_t& texture)
{
	...
	m_Transform = glm::translate(m_Transform, glm::vec3(transform.GetTranslation().x, transform.GetTranslation().y, 1.0f));
	m_Transform = glm::rotate(m_Transform, glm::radians(transform.GetRotation().x), glm::vec3(0.0f, 0.0f, 1.0f));
	m_Transform = glm::scale(m_Transform, glm::vec3(transform.GetScale().x, transform.GetScale().y, 1.0f));
